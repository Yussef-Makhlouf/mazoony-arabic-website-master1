...
export default function EditSheikh() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          setFormData({
            ...
            slug: sheikhData.slug || "",
            city: sheikhData.city || "",
            citySlug: sheikhData.citySlug || "",
            phone: sheikhData.phone || "",
            whatsapp: sheikhData.whatsapp || "",
            rating: sheikhData.rating || 5,
            reviewCount: sheikhData.reviewCount || 0,
            specialties: sheikhData.specialties || [],
            experience: sheikhData.experience || "",
            availability: sheikhData.availability || "متاح",
            bio: sheikhData.bio || "",
            education: sheikhData.education || "",
            languages: sheikhData.languages || [],
          ...
          toast({
            title: "خطأ",
            description: "فشل في تحميل بيانات المأذون",
            variant: "destructive",
          })
        }
      } catch (error) {
        console.error("خطأ في تحميل بيانات المأذون:", error)
        setError("حدث خطأ في تحميل بيانات المأذون")
        toast({
          title: "خطأ",
          description: "حدث خطأ في تحميل بيانات المأذون",
          variant: "destructive",
        })
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        toast({
          title: "نجح",
          description: "تم تحديث المأذون بنجاح",
        })
        router.push("/admin/sheikhs")
      } else {
        const errorData = await response.json()
        throw new Error(errorData.error || "فشل في تحديث المأذون")
      }
    } catch (error: any) {
      console.error("خطأ في تحديث المأذون:", error)
      setError(error.message || "حدث خطأ في تحديث المأذون")
      toast({
        title: "خطأ",
        description: error.message || "حدث خطأ في تحديث المأذون",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
  ...
  if (isLoadingData || isLoadingCities) {
    return (
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">تعديل المأذون</h1>
            <p className="text-gray-600">تعديل بيانات المأذون</p>
          </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-gray-900">تعديل المأذون</h1>
          <p className="text-gray-600">تعديل بيانات المأذون: {formData.name}</p>
        </div>
        <Button variant="outline" asChild>
          <Link href="/admin/sheikhs">
            <X className="w-4 h-4 ml-2" />
